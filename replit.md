...
- **Unlock Keys Schema Groundwork**: Database tables prepared for future "earn keys to unlock topics" feature: `unlockKeys` (tracks earned/used keys per user), `keyUsageHistory` (tracks which topics were unlocked with keys), and `keyUnlocked` field on `topicMastery`.
