...
- **Admin Lesson Regeneration**: Added admin-only "Regenerate" button visible when viewing lesson content. Only the admin account (jpkorstad@gmail.com) can see this button. It clears the lesson content so it regenerates fresh on next access - useful for fixing placeholder or incorrect AI-generated content. Protected by `isAdminUser()` check using the user's email. Endpoints: `GET /api/admin/check` and `POST /api/admin/lessons/:unitId/regenerate`.
